...
void light_compute(vec3 N, vec3 L, vec3 V, float A, vec3 light_color, bool is_directional, uint light_index, float attenuation, vec3 f0, uint orms, float specular_amount, vec3 albedo, inout float alpha,
...
void light_process_omni(uint idx, vec3 vertex, vec3 eye_vec, vec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, vec3 f0, uint orms, float shadow, vec3 albedo, inout float alpha,
	...
	light_compute(normal, normalize(light_rel_vec), eye_vec, size_A, color, false, idx, light_attenuation, f0, orms, omni_lights.data[idx].specular_amount, albedo, alpha,
#ifdef LIGHT_BACKLIGHT_USED
			backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
...
void light_process_spot(uint idx, vec3 vertex, vec3 eye_vec, vec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, vec3 f0, uint orms, float shadow, vec3 albedo, inout float alpha,
	...
	light_compute(normal, normalize(light_rel_vec), eye_vec, size_A, color, false, idx, light_attenuation, f0, orms, spot_lights.data[idx].specular_amount, albedo, alpha,
#ifdef LIGHT_BACKLIGHT_USED
			backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
